...
parser.add_argument('--dataset_pathr', type=str, default='D:\MATLAB_Yan\Test\Mouse_6\ReconDataset', help='path of dataset')
...
###################放入你的模型###################
    ...
    for k in range(11):
        ...
        for epoch in range(args.start_epoch,  args.num_epochs):
            for batch_idx, (rawdata, out_data,A,b) in enumerate((train_loader)): 
            ...
            if epoch % 5 == 0:
                    with torch.no_grad():
                        for batch_idx, (rawdata, out_data) in enumerate((test_loader)): 
                            rawdata = rawdata.to(device)
                            out_data = out_data.to(device)
                            gradup=eng.x_grad(xk_mat,A,b)
                            gradup=torch.from_numpy(np.array(gradup)).float().squeeze().to(device)   
